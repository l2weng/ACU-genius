...
const { array, func, object, oneOf, number, bool, string } = require('prop-types')
...
const ItemTabBody = ({ items, keymap, setPanel, onTagSelect, tab, activeTag, isOwner, mode, ...props }) => {
  switch (true) {
    case (items.length === 0):
      return null
    case (tab === METADATA):
